...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    innogpt: Effect.fnUntraced(function* (input: Info) {
      const env = yield* dep.env()
      const cfg = yield* dep.config()
      const auth = yield* dep.auth(input.id)

      const apiKey = (() => {
        if (env["INNOGPT_API_KEY"]) return env["INNOGPT_API_KEY"]
        if (auth?.type === "api") return auth.key
        const configKey = cfg.provider?.["innogpt"]?.options?.apiKey as string | undefined
      ...
      const configBaseURL = cfg.provider?.["innogpt"]?.options?.baseURL as string | undefined
          ...
          try {
            ...
            for (const m of list) {
              models[m.id] = {
                ...
                providerID: ProviderID.make("innogpt"),
                name: m.id,
                family: "",
                api: {
                  id: m.id,
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        // InnoGPT is not in models.dev, so register a placeholder so the custom loader can run
        if (!database["innogpt"]) {
          database[ProviderID.make("innogpt")] = {
            id: ProviderID.make("innogpt"),
            name: "InnoGPT",
            source: "custom",
            env: ["INNOGPT_API_KEY"],
            options: {
