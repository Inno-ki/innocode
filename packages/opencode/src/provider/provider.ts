...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    gitlab: async (input) => {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `innocode/${Installation.VERSION} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `innocode auth cloudflare-ai-gateway`.",
        )
      }
  ...
  export async function getSmallModel(providerID: ProviderID) {
    ...
    if (innogptProvider && innogptProvider.models["gpt-4o-mini"]) {
      return getModel(ProviderID.innogpt, ModelID.make("gpt-4o-mini"))
    }
