    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: innocode mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in innocode.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .innocode/ and .opencode/ subdirectories too)
  const candidates = [
    path.join(baseDir, "innocode.json"),
    path.join(baseDir, "innocode.jsonc"),
  ...
  if (!global) {
    candidates.push(
      path.join(baseDir, ".innocode", "innocode.json"),
      path.join(baseDir, ".innocode", "innocode.jsonc"),
  ...
  // Default to innocode.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., innocode x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "innocode-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
