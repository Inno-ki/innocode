...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run innocode",
  builder: (yargs) =>
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
        ...
        const bin = Bun.which("innocode") ?? Bun.which("opencode") ?? "innocode"
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for innocode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/innocode\.io\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found innocode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`${bin} import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting innocode...")
        UI.println()

        // Launch innocode TUI with session ID if available
        const { spawn } = await import("child_process")
        const innocodeArgs = sessionId ? ["-s", sessionId] : []
        const innocodeProcess = spawn(bin, innocodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          innocodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`innocode exited with code ${code}`))
          })
          innocodeProcess.on("error", reject)
        })
      },
    })
  },
