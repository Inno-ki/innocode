...
export const USER_AGENT = `innocode/${InstallationChannel}/${InstallationVersion}/${Flag.OPENCODE_CLIENT}`
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const getBrewFormula = Effect.fnUntraced(function* () {
        const tapFormula = yield* text(["brew", "list", "--formula", "inno-ki/tap/innocode"])
        if (tapFormula.includes("innocode")) return "inno-ki/tap/innocode"
        const coreFormula = yield* text(["brew", "list", "--formula", "innocode"])
      ...
      const upgradeCurl = Effect.fnUntraced(
        function* (target: string) {
          const response = yield* httpOk.execute(HttpClientRequest.get("https://innocode.io/install"))
      ...
      const result: Interface = {
        ...
        method: Effect.fn("Installation.method")(function* () {
          if (process.execPath.includes(path.join(".innocode", "bin"))) return "curl" as Method
        ...
        latest: Effect.fn("Installation.latest")(function* (installMethod?: Method) {
          ...
          if (detectedMethod === "brew") {
            ...
            const response = yield* httpOk.execute(
              HttpClientRequest.get("https://formulae.brew.sh/api/formula/innocode.json").pipe(
          ...
          if (detectedMethod === "npm" || detectedMethod === "bun" || detectedMethod === "pnpm") {
            const response = yield* httpOk.execute(
              HttpClientRequest.get(
                `${yield* NpmConfig.registry(process.cwd())}/innocode/${InstallationChannel}`,
          ...
          if (detectedMethod === "scoop") {
            const response = yield* httpOk.execute(
              HttpClientRequest.get(
                "https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/innocode.json",
          ...
          const response = yield* httpOk.execute(
            HttpClientRequest.get("https://api.github.com/repos/Inno-ki/innocode/releases/latest").pipe(
              HttpClientRequest.acceptJson,
            ),
          )
        ...
        upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
          ...
          switch (m) {
            ...
            case "pnpm":
              ...
              break
            case "bun":
              upgradeResult = yield* run(["bun", "install", "-g", `innocode@${target}`])
              break
            case "brew": {
              const formula = yield* getBrewFormula()
              const env = { HOMEBREW_NO_AUTO_UPDATE: "1" }
              if (formula.includes("/")) {
                const tap = yield* run(["brew", "tap", "inno-ki/tap"], { env })
                if (tap.code !== 0) {
                  upgradeResult = tap
                  break
                }
                const repo = yield* text(["brew", "--repo", "inno-ki/tap"])
                const dir = repo.trim()
                if (dir) {
