...
HTTP 基本認証でサーバーを保護するには、`INNOCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `innocode` になるか、`INNOCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`innocode serve` と `innocode web` の両方に当てはまります。
