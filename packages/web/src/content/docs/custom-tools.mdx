---
title: Custom Tools
description: Create tools the LLM can call in innocode.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside innocode's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.innocode/tools/` directory of your project.
- Or globally, by placing them in `~/.config/innocode/tools/`.
...
import { tool } from "@@innogpt/innocode/plugin"
