...
Создайте `.innocode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию innocode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем innocode [config](/docs/config):
...
- Глобальный: `~/.config/innocode/commands/`
- Для каждого проекта: `.innocode/commands/`
...
innocode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
