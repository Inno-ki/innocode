...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start InnoCode.
...
Here are several models that work well with InnoCode, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
Here the full ID is `provider_id/model_id`. For example, if you're using [InnoCode Zen](/docs/zen), you would use `innocode/gpt-5.1-codex` for GPT 5.1 Codex.
