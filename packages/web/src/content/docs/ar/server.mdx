---
title: الخادم
description: تفاعل مع خادم innocode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `innocode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل innocode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `innocode.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `innocode`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `innocode serve` و `innocode web`.
...
عند تشغيل `innocode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم innocode للتفاعل مع innocode برمجيا.
:::

تتيح هذه البنية لـ innocode دعم عدة عملاء وتمكّنك من التفاعل مع innocode برمجيا.

يمكنك تشغيل `innocode serve` لبدء خادم مستقل. إذا كانت واجهة innocode في terminal (TUI)
قيد التشغيل، فسيبدأ `innocode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات InnoCode لـ [IDE](/docs/ide).
...
يعرض خادم innocode واجهات API التالية.
