...
أنشئ الملف `.innocode/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات InnoCode أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ InnoCode:
...
- على مستوى النظام: `~/.config/innocode/commands/`
- لكل مشروع: `.innocode/commands/`
...
يتضمن innocode عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
