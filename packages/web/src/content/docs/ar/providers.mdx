---
title: المزوّدون
description: استخدام أي موفّر LLM في InnoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم InnoCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات InnoCode.
...
في `~/.local/share/innocode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات InnoCode.
...
InnoGPT هي قائمة نماذج يوفّرها فريق InnoCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع InnoCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع InnoGPT.
:::

1. شغّل الأمر `/connect` في TUI، واختر innocode، ثم انتقل إلى [app.innogpt.de](https://app.innogpt.de).
...
يعمل مثل أي مزوّد آخر في InnoCode واستخدامه اختياري بالكامل.
...
لاستخدام Amazon Bedrock مع InnoCode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل innocode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `innocode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل InnoCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل innocode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات innocode.
...
تخزين الرمز في مساحة تخزين مصادقة innocode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم InnoCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر InnoCode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`innocode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [innocode-gitlab-auth](https://www.npmjs.com/package/@gitlab/innocode-gitlab-auth).

##### التهيئة

خصّص عبر `innocode.json`:
...
لاستخدام اشتراك GitHub Copilot مع innocode:
...
لاستخدام Google Vertex AI مع InnoCode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر innocode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [innocode-helicone-session](https://github.com/H2Shami/innocode-helicone-session) لتسجيل كل محادثة InnoCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة InnoCode مدرجة كجلسة مستقلة.
...
يمكنك ضبط innocode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط innocode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط innocode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع InnoCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/innocode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع InnoCode:
...
3. انسخ مفتاح API لاستخدامه في InnoCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في InnoCode، يجب سحب معلومات النموذج محليا:
...
InnoGPT هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق InnoCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>InnoGPT</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **InnoGPT**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ InnoCode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات innocode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات innocode. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع InnoCode:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات innocode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع innocode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `innocode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ InnoCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `innocode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات innocode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات innocode.
