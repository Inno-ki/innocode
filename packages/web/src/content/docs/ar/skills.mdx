...
تتيح مهارات الوكيل لـ InnoCode اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث InnoCode في هذه المواقع:

- إعدادات المشروع: `.innocode/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/innocode/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج InnoCode صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.innocode/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/innocode/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.innocode/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض InnoCode المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `innocode.json`:
...
**للوكلاء المدمجين** (في `innocode.json`):
