...
مع InnoCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم InnoCode سمتنا `innocode`.
...
يأتي InnoCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق InnoCode مظهر terminal لديهم
...
يدعم InnoCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/innocode/themes/*.json` أو `$XDG_CONFIG_HOME/innocode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.innocode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.innocode/themes/*.json`
