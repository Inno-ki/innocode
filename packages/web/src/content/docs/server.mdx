---
title: Server
description: Interact with innocode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `innocode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an innocode client can use.
...
Set `INNOCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `innocode`, or set `INNOCODE_SERVER_USERNAME` to override it. This applies to both `innocode serve` and `innocode web`.
...
When you run `innocode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the innocode server to interact with innocode programmatically.
:::

This architecture lets innocode support multiple clients and allows you to interact with innocode programmatically.

You can run `innocode serve` to start a standalone server. If you have the
innocode TUI running, `innocode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the InnoCode [IDE](/docs/ide) plugins.
...
The innocode server exposes the following APIs.
