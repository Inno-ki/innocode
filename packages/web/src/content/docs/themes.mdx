...
With InnoCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, InnoCode uses our own `innocode` theme.
...
InnoCode comes with several built-in themes.
...
InnoCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/innocode/themes/*.json` or `$XDG_CONFIG_HOME/innocode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.innocode/themes/*.json`
4. **Current working directory** - Defined in `./.innocode/themes/*.json`
