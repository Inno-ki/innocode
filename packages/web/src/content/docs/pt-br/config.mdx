...
Especifique um diretório de configuração personalizado usando a variável de ambiente `OPENCODE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.innocode`, e deve seguir a mesma estrutura.
...
O arquivo de configuração tem um esquema que está definido em [**`innocode.ai/config.json`**](https://innocode.ai/config.json).
