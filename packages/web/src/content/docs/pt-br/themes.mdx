...
Com o innocode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o innocode usa nosso próprio tema `innocode`.
...
O innocode vem com vários temas embutidos.
...
- Querem que o innocode corresponda à aparência do seu terminal
...
O innocode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/innocode/themes/*.json` ou `$XDG_CONFIG_HOME/innocode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.innocode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.innocode/themes/*.json`
