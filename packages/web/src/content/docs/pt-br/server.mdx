...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `innocode`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `innocode serve` quanto ao `innocode web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password innocode serve
```
