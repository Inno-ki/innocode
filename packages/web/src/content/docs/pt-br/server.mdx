---
title: Servidor
description: Interaja com o servidor innocode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `innocode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente innocode pode usar.
...
```bash
innocode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `innocode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
innocode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `INNOCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `innocode`, ou defina `INNOCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `innocode serve` quanto ao `innocode web`.

```bash
INNOCODE_SERVER_PASSWORD=your-password innocode serve
```
...
Quando você executa `innocode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor innocode para interagir com o innocode programaticamente.
:::

Essa arquitetura permite que o innocode suporte múltiplos clientes e permite que você interaja com o innocode programaticamente.

Você pode executar `innocode serve` para iniciar um servidor autônomo. Se você tiver o TUI do innocode em execução, `innocode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do innocode [IDE](/docs/ide).
...
O servidor innocode expõe as seguintes APIs.
