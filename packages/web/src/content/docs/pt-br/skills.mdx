...
As habilidades do agente permitem que o innocode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O innocode pesquisa nesses locais:

- Configuração do projeto: `.innocode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/innocode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o innocode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.innocode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/innocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.innocode/skills/git-release/SKILL.md` assim:
...
O innocode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `innocode.json`:
...
**Para agentes embutidos** (em `innocode.json`):
