---
title: Plugins
description: Escreva seus próprios plugins para estender o innocode.
---

Plugins permitem que você estenda o innocode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do innocode.
...
- `.innocode/plugins/` - Plugins em nível de projeto
- `~/.config/innocode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/innocode/node_modules/`.
...
1. Configuração global (`~/.config/innocode/innocode.json`)
2. Configuração do projeto (`innocode.json`)
3. Diretório de plugins global (`~/.config/innocode/plugins/`)
4. Diretório de plugins do projeto (`.innocode/plugins/`)
...
O innocode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do innocode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o innocode.
...
:::note
Se você estiver usando o aplicativo desktop innocode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o innocode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao innocode:
...
O helper `tool` cria uma ferramenta personalizada que o innocode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o innocode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://innocode.ai/docs/sdk) para detalhes.
