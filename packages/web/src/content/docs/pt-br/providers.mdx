---
title: Provedores
description: Usando qualquer provedor LLM no innocode.
---

import config from "../../../../config.mjs"
export const console = config.console

innocode usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do innocode.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/innocode/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do innocode.
...
InnoGPT é uma lista de modelos fornecidos pela equipe do innocode que foram testados e verificados para funcionar bem com o innocode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o InnoGPT.
:::

1. Execute o comando `/connect` no TUI, selecione innocode e acesse [innocode.ai/auth](https://innocode.ai/auth).
...
Funciona como qualquer outro provedor no innocode e é completamente opcional.
...
Para usar o Amazon Bedrock com o innocode:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   Defina uma dessas variáveis de ambiente ao executar o innocode:
   ...
   Para configuração específica do projeto ou persistente, use `innocode.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no innocode não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o innocode funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do innocode.
...
armazenar o token no armazenamento de autenticação do innocode.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
innocode usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o innocode
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `innocode.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [innocode-gitlab-auth](https://www.npmjs.com/package/@gitlab/innocode-gitlab-auth).

##### Configuração

Personalize através do `innocode.json`:
...
Para usar sua assinatura do GitHub Copilot com o innocode:
...
Para usar o Google Vertex AI com o innocode:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o innocode.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do innocode, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [innocode-helicone-session](https://github.com/H2Shami/innocode-helicone-session) para registrar automaticamente cada conversa do innocode como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do innocode listada como uma sessão separada.
...
Você pode configurar o innocode para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o innocode para usar modelos locais através do LM Studio.
...
Você pode configurar o innocode para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o innocode. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/innocode) para detalhes.
:::
...
Para usar o Ollama Cloud com o innocode:
...
3. Copie a chave da API para uso no innocode.
...
6. **Importante**: Antes de usar modelos em nuvem no innocode, você deve puxar as informações do modelo localmente:
...
InnoGPT é uma lista de modelos testados e verificados fornecidos pela equipe do innocode. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>InnoGPT</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **InnoGPT**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do innocode.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do innocode.
...
5. Você também pode personalizá-los através da sua configuração do innocode. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o innocode:
...
Você também pode personalizar modelos através da sua configuração do innocode. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o innocode. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
1. Execute o comando `/connect` e role para baixo até **Outro**.
   ...
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no innocode.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `innocode.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o innocode entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `innocode auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do innocode e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do innocode.
