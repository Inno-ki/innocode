---
title: Tjener
description: Samhandle med InnoCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `innocode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en InnoCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `innocode.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `innocode` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `innocode serve` og `innocode web`.
...
Når du kjører `innocode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk InnoCode-serveren til å samhandle med InnoCode programmatisk.
:::

Denne arkitekturen lar InnoCode støtte flere klienter og lar deg samhandle med InnoCode programmatisk.

Du kan kjøre `innocode serve` for å starte en frittstående server. Hvis du har
InnoCode TUI kjører, vil `innocode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av InnoCode [IDE](/docs/ide) plugins.
...
innocode-serveren viser følgende APIer.
