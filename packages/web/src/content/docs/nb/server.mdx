...
Sett `INNOCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `innocode` som standard, eller sett `INNOCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `innocode serve` og `innocode web`.
