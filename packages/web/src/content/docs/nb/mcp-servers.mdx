...
Du kan legge til eksterne verktøy i InnoCode ved å bruke _Model Context Protocol_ eller MCP. InnoCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [InnoCode Config](https://innocode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="innocode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/innocode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
InnoCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil InnoCode:
...
Hvis serveren krever autentisering, vil InnoCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `innocode mcp auth <server-name>`.
...
```bash
innocode mcp auth my-oauth-server
```
...
```bash
innocode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil InnoCode lagre tokens sikkert i `~/.local/share/innocode/mcp-auth.json`.
...
innocode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i InnoCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom InnoCode-konfigurasjonen som alle andre verktøy.
...
```json title="innocode.json" {11, 14-18}
{
  "$schema": "https://innocode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
innocode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble InnoCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver InnoCode-samtale oppført som en separat økt.
