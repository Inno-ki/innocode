...
description: Skriv dine egne programtillegg for å utvide innocode.
---

Plugins lar deg utvide InnoCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til InnoCode.
...
- `.innocode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/innocode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/innocode/innocode.json`)
2. Prosjektkonfigurasjon (`innocode.json`)
3. Global plugin-katalog (`~/.config/innocode/plugins/`)
4. Prosjektpluginkatalog (`.innocode/plugins/`)
...
InnoCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En InnoCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide InnoCode.
...
:::note
Hvis du bruker InnoCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre InnoCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til InnoCode:
...
`tool`-hjelperen lager et tilpasset verktøy som InnoCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for InnoCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://innocode.ai/docs/sdk) for detaljer.
