---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i InnoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

InnoCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i InnoCode-konfigurasjonen.
...
i `~/.local/share/innocode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i InnoCode
konfig.
...
InnoGPT er en liste over modeller levert av InnoCode-teamet som har vært
testet og verifisert for å fungere godt med InnoCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med InnoGPT.
:::

1. Kjør kommandoen `/connect` i TUI, velg innocode og gå til [innocode.ai/auth](https://innocode.ai/auth).
...
Det fungerer som alle andre leverandører i InnoCode og er helt valgfritt å bruke.
...
Slik bruker du Amazon Bedrock med InnoCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører InnoCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `innocode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i innocode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at InnoCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom InnoCode-konfigurasjonen.
...
for å lagre token i InnoCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
InnoCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse InnoCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`innocode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [innocode-gitlab-auth](https://www.npmjs.com/package/@gitlab/innocode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `innocode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med InnoCode:
...
Slik bruker du Google Vertex AI med InnoCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører InnoCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom InnoCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [innocode-helicone-session](https://github.com/H2Shami/innocode-helicone-session) plugin for automatisk å logge hver InnoCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver InnoCode-samtale oppført som en separat økt.
...
Du kan konfigurere InnoCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere InnoCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere InnoCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for InnoCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/innocode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med InnoCode:
...
3. Kopier API-nøkkelen for bruk i InnoCode.
...
6. **Viktig**: Før du bruker skymodeller i InnoCode, må du hente modellinformasjonen lokalt:
...
InnoGPT er en liste over testede og verifiserte modeller levert av InnoCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>InnoGPT</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **InnoGPT**.

   ```txt
   /connect
   ```

3. Skriv inn InnoCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom InnoCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom InnoCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med InnoCode:
...
Du kan også tilpasse modeller gjennom InnoCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med InnoCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `innocode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar InnoCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `innocode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk InnoCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i InnoCode-konfigurasjonen.
