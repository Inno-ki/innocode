...
Det er to typer agenter i InnoCode; primære agenter og underagenter.
...
InnoCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
InnoCode kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

InnoCode kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `innocode.json`:
...
- Globalt: `~/.config/innocode/agents/`
- Per prosjekt: `.innocode/agents/`
...
Hvis ingen temperatur er spesifisert, bruker InnoCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale InnoCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i InnoCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [InnoGPT](/docs/zen), vil du bruke `innocode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `innocode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/innocode).
:::
