...
In InnoCode esistono due tipi di agenti: agenti primari e subagenti.
...
InnoCode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
InnoCode include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

InnoCode include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `innocode.json`:
...
- Globale: `~/.config/innocode/agents/`
- Per progetto: `.innocode/agents/`
...
Se non specifichi una temperature, InnoCode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di InnoCode sia per la config specifica del progetto.
...
L'ID modello nella configurazione InnoCode usa il formato `provider/model-id`. Per esempio, se stai usando [InnoGPT](/docs/zen), useresti `innocode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `innocode models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/innocode).
:::
