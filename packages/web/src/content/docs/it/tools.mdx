...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. InnoCode include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in InnoCode.
...
:::note
Questo strumento e' disponibile solo quando `INNOCODE_EXPERIMENTAL_LSP_TOOL=true` (oppure `INNOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider InnoCode o quando la variabile d'ambiente `INNOCODE_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di InnoCode:
