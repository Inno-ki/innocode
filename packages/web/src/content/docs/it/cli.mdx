...
Avvia un server HTTP che espone accesso API alle funzionalità di innocode senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `innocode`).
...
Avvia un server HTTP e apre un browser per accedere a InnoCode tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `innocode`).
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `innocode`)     |
