...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `OPENCODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.innocode` e dovrebbe seguire la stessa struttura.
...
Il file di configurazione ha uno schema definito in [**`innocode.ai/config.json`**](https://innocode.ai/config.json).
