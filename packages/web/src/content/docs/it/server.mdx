...
Imposta `INNOCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `innocode`, oppure imposta `INNOCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `innocode serve` sia per `innocode web`.
