...
Le skill degli agenti permettono a InnoCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
InnoCode cerca in queste posizioni:

- Config di progetto: `.innocode/skills/<name>/SKILL.md`
- Config globale: `~/.config/innocode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, InnoCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.innocode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/innocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.innocode/skills/git-release/SKILL.md` cosi':
...
InnoCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `innocode.json`:
...
**Per agenti integrati** (in `innocode.json`):
