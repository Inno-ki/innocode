...
Puoi aggiungere strumenti esterni a InnoCode usando il _Model Context Protocol_, o MCP. InnoCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [InnoCode Config](https://innocode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/innocode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
InnoCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, InnoCode:
...
Se il server richiede autenticazione, InnoCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `innocode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, InnoCode memorizzera' i token in modo sicuro in `~/.local/share/innocode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in InnoCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione InnoCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare InnoCode al tuo account Sentry.
