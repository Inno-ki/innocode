...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di innocode. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in innocode ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di innocode.
...
innocode include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.innocode/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `innocode.json`:
...
- Globale: `~/.config/innocode/modes/`
- Progetto: `.innocode/modes/`
...
Se non viene specificata alcuna temperatura, innocode usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di innocode sia per quella specifica del progetto.
...
Crea file di modalita in `.innocode/modes/` per modalita specifiche del progetto oppure in `~/.config/innocode/modes/` per modalita globali:
