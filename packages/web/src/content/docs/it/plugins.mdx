---
title: Plugin
description: Scrivi plugin per estendere InnoCode.
---

I plugin ti permettono di estendere InnoCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalita', integrare servizi esterni o modificare il comportamento predefinito di InnoCode.
...
- `.innocode/plugins/` - plugin a livello progetto
- `~/.config/innocode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/innocode/node_modules/`.
...
1. Config globale (`~/.config/innocode/innocode.json`)
2. Config di progetto (`innocode.json`)
3. Directory plugin globale (`~/.config/innocode/plugins/`)
4. Directory plugin di progetto (`.innocode/plugins/`)
...
InnoCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di innocode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere innocode.
...
:::note
Se usi l'app desktop di InnoCode, puo' inviare automaticamente notifiche di sistema quando una risposta e' pronta o quando una sessione va in errore.
:::
...
Impedisci a innocode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a innocode:
...
L'helper `tool` crea uno strumento personalizzato che innocode puo' chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in innocode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://innocode.ai/docs/sdk) per i dettagli.
