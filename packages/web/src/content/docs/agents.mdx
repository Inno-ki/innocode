...
There are two types of agents in InnoCode; primary agents and subagents.
...
InnoCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
InnoCode comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

InnoCode comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `innocode.json` config file:
...
If no temperature is specified, InnoCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global InnoCode config and the project specific config.
...
The model ID in your InnoCode config uses the format `provider/model-id`. For example, if you're using [InnoCode Zen](/docs/zen), you would use `innocode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `innocode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/Inno-ki/innocode).
:::
