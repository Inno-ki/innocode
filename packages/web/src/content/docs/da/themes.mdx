...
Med InnoCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger InnoCode vårt eget `innocode`-tema.
...
InnoCode kommer med flere innebygde temaer.
...
- Vil at InnoCode skal matche terminalens utseende
...
InnoCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/innocode/themes/*.json` eller `$XDG_CONFIG_HOME/innocode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.innocode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.innocode/themes/*.json`
