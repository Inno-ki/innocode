---
title: Server
description: Interager med innocode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `innocode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en innocode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `innocode.local` |
...
Indstil `INNOCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `innocode`, eller indstil `INNOCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `innocode serve` og `innocode web`.
...
Når du kører `innocode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug innocode-serveren til at interagere med innocode programmatisk.
:::

Denne arkitektur lader innocode understøtte flere klienter og lader dig interagere med innocode programmatisk.

Du kan køre `innocode serve` for at starte en selvstændig server. Hvis du har
innocode TUI kørende, vil `innocode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af InnoCode [IDE](/docs/ide) plugins.
...
InnoCode-serveren viser følgende API'er.
