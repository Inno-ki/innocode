---
title: Plugins
description: Skriv dine egne plugins for at udvide InnoCode.
---

Plugins giver dig mulighed for at udvide InnoCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre InnoCodes standardadfærd.
...
- `.innocode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/innocode/node_modules/`.
...
1. Global konfiguration (`~/.config/innocode/innocode.json`)
2. Projektkonfiguration (`innocode.json`)
3. Global plugin-mappe (`~/.config/innocode/plugins/`)
4. Projekt plugin bibliotek (`.innocode/plugins/`)
...
InnoCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En innocode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide innocode.
...
:::note
Hvis du bruger InnoCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå innocode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til innocode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som innocode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for innocode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://innocode.ai/docs/sdk) for detaljer.
