...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i innocode-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i innocode giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem innocode-konfigurationen.
...
innocode leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.innocode/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `innocode.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger innocode modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale innocode-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.innocode/modes/` for projektspecifikke tilstande eller `~/.config/innocode/modes/` for globale tilstande:
