...
Der er to typer agenter i InnoCode; primære agenter og subagenter.
...
InnoCode leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
InnoCode leveres med to indbyggede underagenter, **Generelt** og **Udforsk**. Vi vil se på dette nedenfor.

---

## Indbyggede

InnoCode leveres med to indbyggede primære agenter og to indbyggede subagenter.
...
Konfigurer agent i din `innocode.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger InnoCode modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale InnoCode-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din InnoCode-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [InnoGPT](/docs/zen), vil du bruge `innocode/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `innocode models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/anomalyco/innocode).
:::
