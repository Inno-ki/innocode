...
Agentferdigheter lar InnoCode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
InnoCode søker etter disse stedene:

- Prosjektkonfigurasjon: `.innocode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/innocode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går InnoCode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.innocode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/innocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.innocode/skills/git-release/SKILL.md` slik:
...
InnoCode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `innocode.json`:
...
**For innebygde agenter** (i `innocode.json`):
