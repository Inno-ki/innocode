...
For at fejlfinde problemer med InnoCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\innocode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `innocode --log-level DEBUG`.

---

## Lagring

innocode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/innocode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\innocode`
...
InnoCode Desktop kører en lokal InnoCode-server (`innocode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `InnoCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/innocode/innocode.jsonc` (eller `~/.config/innocode/innocode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/innocode/innocode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\innocode\innocode.jsonc`
...
InnoCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/innocode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\innocode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så InnoCode kan genopbygge den.

1. Afslut InnoCode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/innocode`
- **Linux**: slet `~/.cache/innocode` (eller kør `rm -rf ~/.cache/innocode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\innocode`

3. Start InnoCode Desktop på ny.

---

### Løs problemer med serverforbindelse

InnoCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `innocode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `INNOCODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `INNOCODE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver InnoCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i InnoCode.

---

### Meddelelser vises ikke

InnoCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for InnoCode i dine OS-indstillinger, og
...
1. Afslut InnoCode Desktop.
2. Find og slet disse filer (de findes i InnoCode Desktop-appens datamappe):

- `innocode.settings.dat` (skrivebordsstandardserver URL)
- `innocode.global.dat` og `innocode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med InnoCode:
...
For at finde ud af hvilke modeller du har adgang til, kør `innocode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\innocode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. innocode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\innocode`

2. Start innocode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge innocode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
innocode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
