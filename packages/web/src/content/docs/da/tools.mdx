...
Her er alle de innebygde verktøyene som er tilgængelige i InnoCode.
...
:::note
Dette verktøyet er bare tilgængelig når `INNOCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `INNOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger InnoCode-leverandøren eller når miljøvariabelen `INNOCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
