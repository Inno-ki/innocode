---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i InnoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

InnoCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i InnoCode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/innocode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i InnoCode-konfigurationen.
...
InnoGPT er en liste over modeller leveret af InnoCode-teamet, som er blevet testet og verificeret til at fungere godt med InnoCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med InnoGPT.
:::

1. Kør kommandoen `/connect` i TUI, vælg innocode og gå til [innocode.ai/auth](https://innocode.ai/auth).
...
Det fungerer som alle andre udbydere i InnoCode og er helt valgfrit at bruge.
...
Sådan bruger du Amazon Bedrock med InnoCode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører innocode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `innocode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i InnoCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at innocode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem innocode-konfigurationen.
...
gemme tokenet i innocode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
InnoCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse InnoCode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`innocode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [innocode-gitlab-auth](https://www.npmjs.com/package/@gitlab/innocode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `innocode.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med InnoCode:
...
Sådan bruger du Google Vertex AI med InnoCode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører innocode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem innocode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [innocode-helicone-session](https://github.com/H2Shami/innocode-helicone-session) plugin for automatisk at logge hver InnoCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver InnoCode-samtale opført som en separat session.
...
Du kan konfigurere innocode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere innocode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere innocode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for InnoCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/innocode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med InnoCode:
...
3. Kopier API-nøglen til brug i InnoCode.
...
6. **Vigtigt**: Før du bruger skymodeller i InnoCode, skal du hente modelinformationen lokalt:
...
InnoGPT er en liste over testede og verificerede modeller leveret af InnoCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>InnoGPT</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **InnoGPT**.

   ```txt
   /connect
   ```

3. Indtast InnoCode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem innocode-konfigurationen.
...
5. Du kan også tilpasse dem gennem innocode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med InnoCode:
...
Du kan også tilpasse modeller gennem innocode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med innocode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `innocode.json`-filen i projektkataloget:
...
`limit`-felterne lader InnoCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `innocode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek innocode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i innocode-konfigurationen.
