...
InnoCode is powered by the provider list at [Models.dev](https://models.dev), so you can use `innocode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/innocode/auth.json`.

```bash
innocode auth login
```

When InnoCode starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
This starts an HTTP server that provides API access to innocode functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `innocode`).
...
This starts an HTTP server and opens a web browser to access InnoCode through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `innocode`).
...
| `OPENCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `innocode`) |
