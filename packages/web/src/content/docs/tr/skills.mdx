...
Ajan becerileri, innocode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
innocode su konumlari tarar:

- Proje konfigurasyonu: `.innocode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/innocode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda innocode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.innocode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/innocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.innocode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
innocode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`innocode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`innocode.json` icinde):
