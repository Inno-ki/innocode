---
title: Eklentiler
description: innocode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek innocode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya innocode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.innocode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/innocode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/innocode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/innocode/innocode.json`)
2. Project config (`innocode.json`)
3. Global eklenti dizini (`~/.config/innocode/plugins/`)
4. Proje eklenti dizini (`.innocode/plugins/`)
...
innocode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşime geçmek için innocode'lu bir SDK istemcisi.
...
innocode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
innocode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
innocode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca innocode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, innocode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK documentation](https://innocode.ai/docs/sdk)'e bakın.
