...
_Model Bağlam Protokolü_ veya MCP kullanarak innocode'a harici araçlar ekleyebilirsiniz. innocode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [innocode Config](https://innocode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/innocode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
innocode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde innocode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa innocode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `innocode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından innocode, belirteçleri `~/.local/share/innocode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra innocode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi innocode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve innocode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
