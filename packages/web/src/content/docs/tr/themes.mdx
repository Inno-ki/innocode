...
innocode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak innocode kendi `innocode` temasını kullanır.
...
innocode birden fazla yerleşik temayla gelir.
...
- innocode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
innocode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/innocode/themes/*.json` veya `$XDG_CONFIG_HOME/innocode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.innocode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.innocode/themes/*.json`
