...
description: innocode, LSP sunucularınızla bütünleşir.
---

innocode, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

innocode, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`INNOCODE_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
InnoCode bir dosyayı açtığında:
...
LSP sunucularını, innocode yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
