...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `INNOCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `innocode` değeridir; değiştirmek isterseniz `INNOCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `innocode serve` hem de `innocode web` için geçerlidir.
