...
:::caution
Modlar artık innocode yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

innocode'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya innocode yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
innocode iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.innocode/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`innocode.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, innocode modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global innocode yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.innocode/modes/` veya genel modlar için `~/.config/innocode/modes/`'de mod dosyaları oluşturun:
