...
innocode ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\innocode\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `innocode --log-level DEBUG`.

---

## Depolama

innocode, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/innocode/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\innocode` yapistirin
...
innocode Desktop arka planda yerel bir innocode sunucusu (`innocode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `innocode` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/innocode/innocode.jsonc` (veya `~/.config/innocode/innocode.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/innocode/innocode.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\innocode\innocode.jsonc` yapistirin
...
innocode diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/innocode/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\innocode\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), innocode'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. innocode Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/innocode`
- **Linux**: `~/.cache/innocode` dizinini silin (veya `rm -rf ~/.cache/innocode` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\innocode` yapistirin

3. innocode Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

innocode Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`innocode.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `INNOCODE_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `INNOCODE_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta innocode Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, innocode ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

innocode Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda innocode icin bildirimler etkinse
...
1. innocode Desktop'u kapatin
2. Su dosyalari bulun ve silin (innocode Desktop uygulama veri dizininde yer alirlar):

- `innocode.settings.dat` (desktop varsayilan sunucu URL'si)
- `innocode.global.dat` ve `innocode.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
innocode ile ilgili bir sorun yasiyorsaniz:
...
3. `innocode upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `innocode models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\innocode`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. innocode, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\innocode`

2. En guncel provider paketlerini yeniden kurmak icin innocode'u yeniden baslatin

Bu, innocode'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
innocode, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
