...
innocode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
innocode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
innocode, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

innocode iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `innocode.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse innocode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global innocode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
innocode hesabınızdaki model kimliğini `provider/model-id` biçimini kullanır. Örneğin, [InnoGPT](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `innocode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `innocode models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/innocode).
:::
