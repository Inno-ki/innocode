...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. innocode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
innocode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `INNOCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `INNOCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca innocode provider kullanılırken veya `INNOCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

innocode başlatırken etkinleştirmek için:
