---
title: 分享
description: 分享您的 innocode 對話。
---

innocode 的共享功能允許您建立 innocode 對話的公共連結，以便您可以與團隊成員協作或從其他人那裡獲得幫助。
...
3. 透過可共享連結進行對話 — `innocode.io/s/<share-id>`

---

## 模式

innocode 支援三種控制對話共享方式的共享模式：

---

### 手動（預設）

innocode 預設使用手動共享模式。工作階段不會自動共享，但您可以使用 `/share` 指令手動共享它們：
...
要在整個團隊中針對給定專案強制執行此操作，請將其添加到專案中的 `innocode.json` 並簽入 Git。
...
[了解更多](/docs/enterprise) 關於在您的組織中使用 innocode。
