...
這將啟動一個 HTTP 伺服器，該伺服器提供對 innocode 功能的 API 存取，而無需 TUI 介面。設定 `OPENCODE_SERVER_PASSWORD` 以啟用 HTTP 基本身分驗證（使用者名稱預設為 `innocode`）。
...
這將啟動 HTTP 伺服器並打開網頁瀏覽器以透過 Web 介面存取 innocode。設定 `OPENCODE_SERVER_PASSWORD` 以啟用 HTTP 基本身分驗證（使用者名稱預設為 `innocode`）。
...
| `OPENCODE_SERVER_USERNAME`            | string  | 覆蓋基本身分驗證使用者名稱（預設 `innocode`） |
