...
代理技能讓 innocode 從您的儲存庫或主目錄中發現可重複使用的指令。
...
- 專案設定：`.innocode/skills/<name>/SKILL.md`
- 全域設定：`~/.config/innocode/skills/<name>/SKILL.md`
...
對於專案本地路徑，innocode 將從當前工作目錄開始，直到到達 git 工作樹。
它載入 `skills/*/SKILL.md` 中任何匹配的 `.innocode/` 以及一路上任何匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。

全域定義也從 `~/.config/innocode/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md` 和 `~/.agents/skills/*/SKILL.md` 載入。
...
像這樣建立 `.innocode/skills/git-release/SKILL.md`：
...
innocode 在 `skill` 工具描述中列出了可用的技能。
每個項目都包含技能名稱和描述：
...
使用 `innocode.json` 中基於模式的權限控制代理可以存取哪些技能：
