...
使用 `OPENCODE_CONFIG_DIR` 環境變數指定自定義設定目錄。將在該目錄中搜尋代理、指令、模式和外掛，就像標準 `.innocode` 目錄一樣，並且應該遵循相同的結構。
...
設定檔具有在 [**`innocode.ai/config.json`**](https://innocode.ai/config.json) 中定義的架構。
