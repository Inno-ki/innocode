---
title: Providers
description: Using any LLM provider in InnoCode.
---

import config from "../../../config.mjs"
export const console = config.console

InnoCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your InnoCode config.
...
You can customize the providers through the `provider` section in your InnoCode
config.
...
InnoGPT is a list of models provided by the InnoCode team that have been
tested and verified to work well with InnoCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with InnoGPT.
:::

1. Run the `/connect` command in the TUI, select innocode, and head to [innocode.ai/auth](https://innocode.ai/auth).
...
2. **Configure authentication** using one of the following methods:
   ...
   For project-specific or persistent configuration, use `innocode.json`:
...
:::info
Using your Claude Pro/Max subscription in InnoCode is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for innocode to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your innocode config.
...
InnoCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock InnoCode
to only use your own GitLab-hosted instance, add the following to your
`innocode.json` file. It is also recommended to disable session sharing.
...
More documentation on [innocode-gitlab-auth](https://www.npmjs.com/package/@gitlab/innocode-gitlab-auth) homepage.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [innocode-helicone-session](https://github.com/H2Shami/innocode-helicone-session) plugin to automatically log each InnoCode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each InnoCode conversation listed as a separate session.
...
You can configure innocode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
:::tip
Ollama can automatically configure itself for InnoCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/innocode) for details.
:::
...
3. Copy the API key for use in InnoCode.
...
6. **Important**: Before using cloud models in InnoCode, you must pull the model information locally:
...
InnoGPT is a list of tested and verified models provided by the InnoCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>InnoGPT</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **InnoGPT**.

   ```txt
   /connect
   ```

3. Enter your InnoCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your innocode config.
...
5. You can also customize them through your innocode config. Here's an example of specifying a provider
...
:::tip
You can use any OpenAI-compatible provider with innocode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `innocode.json` file in your project directory:
...
1. **Check the auth setup**: Run `innocode auth list` to see if the credentials
...
2. For custom providers, check the innocode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your innocode config.
