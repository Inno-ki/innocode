...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `innocode`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `innocode serve` and `innocode web`.
