...
使用 `OPENCODE_CONFIG_DIR` 环境变量指定自定义配置目录。将在该目录中搜索代理、命令、模式和插件，就像标准 `.innocode` 目录一样，并且应该遵循相同的结构。
...
配置文件具有在 [**`innocode.ai/config.json`**](https://innocode.ai/config.json) 中定义的架构。
