---
title: 提供商
description: 在 innocode 中使用任意 LLM 提供商。
---

import config from "../../../../config.mjs"
export const console = config.console

innocode 使用 [AI SDK](https://ai-sdk.dev/) 和 [Models.dev](https://models.dev)，支持 **75+ LLM 提供商**，也支持运行本地模型。

要添加提供商，你需要：

1. 使用 `/connect` 命令添加提供商的 API 密钥。
2. 在 innocode 配置中设置该提供商。
...
`~/.local/share/innocode/auth.json`。

---

### 配置

你可以使用 innocode 配置中的 `provider` 部分自定义提供商配置。
...
InnoGPT 是innocode团队提供的模型列表，这些模型已被
经测试和验证可与innocode良好配合。 [了解更多](/docs/zen)。

:::tip
如果您是新手，我们建议您从 InnoGPT 开始。
:::

1. Run the `/connect` command in the TUI, select innocode, and head to [app.innogpt.de](https://app.innogpt.de).
...
它的工作方式与 innocode 中的任何其他提供的程序相同，并且完全可以选择使用。
...
相当于 Amazon Bedrock 与 innocode 结合使用：
...
将Tokens存储在innocode身份验证存储中。
:::

##### 自托管 GitLab

:::note[合规笔记]
innocode 使用一个小模型来执行一些 AI 任务，例如生成会话标题。
情况下，其配置为使用 gpt-5-nano，由 Zen 托管。默认 innocode
...
相当于您的 GitHub Copilot 订阅与 innocode 一起使用：
...
Google Vertex AI 与 innocode 结合使用：
...
如果您发现Helicone的某些功能或模型未通过innocode自动配置，您始终可以自行配置。
...
该外挂将 `Helicone-Session-Id` 和 `Helicone-Session-Name` 标头注入您的请求中。在 Helicone 的会话页面中，您将看到每个 innocode 对话都是单独的会话。
...
您可以通过使用本地模型来使用 LM Studio 配置innocode。
...
6. **重要**：在innocode中使用云模型之前，必须将模型信息拉取到本地：
...
InnoGPT 是 innocode 团队提供的经过测试和验证的模型列表。 [了解更多](/docs/zen)。

1. 登录 **<a href={console}>InnoGPT</a>** 并单击 **创建 API 密钥**。

2. 执行 `/connect` 命令并搜索 **InnoGPT**。

   ```txt
   /connect
   ```

3. 输入您的 innocode API 密钥。
...
5. 您还可以使用innocode配置自定义它们。这是指定的示例
...
您还可以穿透 innocode 配置自定义模型。以下是指定提供商路由顺序的示例。
...
:::tip
您可以将任何 OpenAI 兼容的提供商与 innocode 一起使用。大多数 AI 提供商都提供 OpenAI 兼容 API。
:::
...
`limit` 栏位允许 innocode 了解您还剩下多少上下文。标准提供商会自动从 models.dev 中提取这些内容。
...
2. 对于自定义提供的程序，请检查 innocode 配置并：
   - 确保 `/connect` 命令中使用的提供商 ID 与 innocode 配置中的 ID 匹配。
