...
要排除 innocode 的问题，请首先检查其存储在磁碟上的日志和本地数据。
...
innocode程序将会话数据和其他应用程序数据存储在磁碟上：
...
innocode 还可以从磁碟加载本地外挂。暂时将它们移开（或重新命名资料夹）并重新启动桌面应用程序：
...
如果取消外挂没有帮助（或者外挂安装被卡住），请清除缓存，方便innocode可以重建它。
...
innocode Desktop 可以启动自己的本地服务器（默认配置）或连线到您的服务器 URL。
...
如果您在环境中设置了 `INNOCODE_PORT`，桌面应用程序将尝试将交换机用于本地服务器。

- 取消设置`INNOCODE_PORT`（或选择一个休闲摊）并重新启动。
...
在 Windows 上，innocode 桌面需要 Microsoft Edge **WebView2 执行时**。如果应用程序打开为空白视窗或无法启动，请 install/update WebView2 并重试。
...
innocode 桌面仅在以下情况下显示系统通知：

- 在您的作业系统中设置为 innocode 启用了通知，并且
- 应用程序視窗未聚焦。
...
1. 退出 innocode 桌面。
2. 查询并删除这些文件（它们位于 innocode 桌面应用程序数据目录中）：
...
如果您遇到 API 呼叫错误，这可能是由于过去提供包造成的。 innocode 根据需要动态安装提供包（OpenAI、Anthropic、Google 等）将其缓存放在本地。
...
2. 重新启动 innocode 以重新安装最新的提供包

这将需要 innocode 下载最新版本的提供包，这通常可以解决模型参数和 API 更改的兼容性问题。
...
innocode 将检测您是否正在使用 Wayland 并更喜欢 `wl-clipboard`，否则将尝试按以下顺序剪贴簿工具：`xclip` 和 `xsel`。
