...
这将启动一个 HTTP 服务器，该服务器提供对 innocode 功能的 API 访问，无需 TUI 界面。设置 `OPENCODE_SERVER_PASSWORD` 以启用 HTTP 基本身份验证（用户名默认为 `innocode`）。
...
这将启动 HTTP 服务器并打开 Web 浏览器以通过 Web 界面访问 innocode。设置 `OPENCODE_SERVER_PASSWORD` 以启用 HTTP 基本身份验证（用户名默认为 `innocode`）。
...
| `OPENCODE_SERVER_USERNAME`            | string  | 覆盖基本身份验证用户名（默认 `innocode`） |
