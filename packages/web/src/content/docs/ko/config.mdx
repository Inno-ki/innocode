...
`OPENCODE_CONFIG_DIR` 환경 변수를 사용하여 사용자 정의 구성 디렉토리를 지정할 수 있습니다. 이 디렉토리는 표준 `.innocode` 디렉토리와 마찬가지로 에이전트, 명령, 모드 및 플러그인을 검색하며 동일한 구조를 따라야 합니다.
...
구성 파일에는 [**`innocode.ai/config.json`**](https://innocode.ai/config.json)에 정의된 스키마가 있습니다.
