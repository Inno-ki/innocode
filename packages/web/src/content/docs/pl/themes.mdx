...
Dzięki innocode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie innocode używa naszego własnego motywu `innocode`.
...
innocode ma kilka wbudowanych motywów.
...
- Want innocode to match their terminal's appearance
...
innocode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/innocode/themes/*.json` lub `$XDG_CONFIG_HOME/innocode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.innocode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.innocode/themes/*.json`
