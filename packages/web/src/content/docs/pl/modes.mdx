...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji innocode.
...
te lub skonfiguruj własne za pomocą konfiguracji innocode.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.innocode/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `innocode.json`:
...
- Globalnie: `~/.config/innocode/modes/`
...
Jeśli nie ma wpływu na temperaturę, innocode stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna innocode, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.innocode/modes/` dla trybów zapisanych dla projektu lub `~/.config/innocode/modes/` dla trybów globalnych:
