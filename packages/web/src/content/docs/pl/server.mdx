---
title: Serwer
description: Interakcja z serwerem innocode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `innocode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient innocode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `innocode.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `innocode` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `innocode serve`, jak i `innocode web`.
...
Kiedy uruchomisz `innocode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera innocode do programowej interakcji z innocode.
:::

Ta architektura umożliwia obsługę wielu klientów przez innocode i programową interakcję z kodem otwartym.

Możesz uruchomić `innocode serve`, aby uruchomić samodzielny serwer. Jeśli masz
innocode TUI działa, `innocode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki innocode [IDE](/docs/ide).
...
Serwer innocode udostępnia następujące interfejsy API.
