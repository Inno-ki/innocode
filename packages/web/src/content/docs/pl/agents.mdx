...
W InnoCode obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
InnoCode zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
InnoCode ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

InnoCode ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `innocode.json`:
...
- Globalnie: `~/.config/innocode/agents/`
- W projekcie: `.innocode/agents/`
...
Jeśli nie zostanie skonfigurowana, InnoCode używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji InnoCode, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji InnoCode w `provider/model-id`. Na przykład, [InnoGPT](/docs/zen), użyj `innocode/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `innocode models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/innocode).
:::
