...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. InnoCode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w InnoCode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `INNOCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `INNOCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy InnoCode lub gdy zmienna środowiskowa `INNOCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu InnoCode:
