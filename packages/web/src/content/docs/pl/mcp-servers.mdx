...
Możesz dodać dodatkowe informacje do innocode za pomocą _Model Context Protocol_, w skrócie MCP. innocode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [innocode Config](https://innocode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/innocode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
innocode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, innocode:
...
Jeśli serwer wymaga uwierzytelnienia, innocode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `innocode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji innocode bezpieczne przechowa tokeny w `~/.local/share/innocode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w innocode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie innocode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć innocode z kontem Sentry.
