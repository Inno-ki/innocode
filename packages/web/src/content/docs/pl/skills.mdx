...
Umiejętności agenta pozwalają innocode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
innocode przeszukuje te lokalizacje:

- Project config: `.innocode/skills/<name>/SKILL.md`
- Global config: `~/.config/innocode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu innocode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.innocode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/innocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.innocode/skills/git-release/SKILL.md` w ten sposób:
...
innocode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `innocode.json`:
...
**Dla agentów wbudowanych** (w `innocode.json`):
