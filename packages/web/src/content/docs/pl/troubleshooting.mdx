...
Aby debugować problemy z innocode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\innocode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `innocode --log-level DEBUG`.

---

## Przechowywanie danych

innocode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/innocode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\innocode`
...
innocode Desktop uruchamia w tle lokalny serwer innocode (sidecar `innocode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `InnoCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/innocode/innocode.jsonc` (lub `~/.config/innocode/innocode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/innocode/innocode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\innocode\innocode.jsonc`
...
innocode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/innocode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\innocode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby innocode mógł ją odbudować.

1. Całkowicie zamknij innocode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/innocode`
- **Linux**: usuń `~/.cache/innocode` (lub uruchom `rm -rf ~/.cache/innocode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\innocode`

3. Uruchom ponownie innocode Desktop.

---

### Napraw problemy z połączeniem z serwerem

innocode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `innocode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `INNOCODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `INNOCODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows innocode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami innocode.

---

### Brak powiadomień

innocode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla innocode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie innocode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji innocode Desktop):

- `innocode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `innocode.global.dat` i `innocode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z innocode:
...
3. Upewnij się, że masz najnowszą wersję z `innocode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `innocode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\innocode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. innocode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\innocode`

2. Uruchom ponownie kod innocode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to innocode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
innocode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
