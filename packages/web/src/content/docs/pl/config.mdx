...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `OPENCODE_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.innocode` i powinien zachować tę samą strukturę.
...
Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`innocode.ai/config.json`**](https://innocode.ai/config.json).
