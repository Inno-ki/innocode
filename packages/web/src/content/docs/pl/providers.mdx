---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w innocode.
---

import config from "../../../../config.mjs"
export const console = config.console

innocode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji innocode.
...
w `~/.local/share/innocode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim innocode
config.
...
InnoGPT to lista modeli dostarczonych przez zespół innocode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z innocode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od InnoGPT.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz innocode i przejdź do [innocode.ai/auth](https://innocode.ai/auth).
...
Działa jak każdy inny dostawca w innocode i jest całkowicie opcjonalny w użyciu.
...
Aby używać Amazon Bedrock z innocode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania innocode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `innocode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w innocode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod innocode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji innocode.
...
innocode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować innocode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`innocode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [innocode-gitlab-auth](https://www.npmjs.com/package/@gitlab/innocode-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem innocode:
...
Aby używać Google Vertex AI z innocode:
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą innocode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [innocode-helicone-session](https://github.com/H2Shami/innocode-helicone-session), aby automatycznie rejestrować każdą konwersację innocode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja innocode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować innocode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować innocode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować innocode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem innocode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/innocode).
:::
...
Aby korzystać z Ollama Cloud z innocode:
...
3. Skopiuj klucz API do użycia w innocode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w innocode musisz pobrać informacje o modelu lokalnie:
...
InnoGPT to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół innocode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>InnoGPT</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **InnoGPT**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API innocode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji innocode.
...
5. Możesz je także dostosować za pomocą konfiguracji innocode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z innocode:
...
Możesz także dostosować modele za pomocą konfiguracji innocode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z innocode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `innocode.json` w katalogu projektu:
...
Pola `limit` pozwalają innocode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `innocode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację innocode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji innocode.
