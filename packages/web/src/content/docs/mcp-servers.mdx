...
You can add external tools to InnoCode using the _Model Context Protocol_, or MCP. InnoCode supports both local and remote servers.
...
You can define MCP servers in your [InnoCode Config](https://innocode.io/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/innocode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
InnoCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, InnoCode will:
...
If the server requires authentication, InnoCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `innocode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, InnoCode will store the tokens securely in `~/.local/share/innocode/mcp-auth.json`.
...
Your MCPs are available as tools in InnoCode, alongside built-in tools. So you can manage them through the InnoCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect InnoCode to your Sentry account.
