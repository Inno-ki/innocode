...
:::caution
Modes are now configured through the `agent` option in the innocode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in innocode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the innocode config.
...
innocode comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.innocode/plans/*.md` to detail the plan itself
...
Configure modes in your `innocode.json` config file:
...
both the global innocode config and the project specific config.
...
Create mode files in `.innocode/modes/` for project-specific modes or `~/.config/innocode/modes/` for global modes:
