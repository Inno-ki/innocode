...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. InnoCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u InnoCode.
...
:::note
Ovaj alat je dostupan samo kada je `INNOCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `INNOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz InnoCode provajdera ili kada je varijabla `INNOCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju InnoCode:
