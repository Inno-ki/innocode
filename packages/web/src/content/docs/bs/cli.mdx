...
Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti InnoCode-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `innocode`).
...
Ovo pokreće HTTP server i otvara web pretraživač za pristup InnoCode-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `innocode`).
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `innocode`) |
