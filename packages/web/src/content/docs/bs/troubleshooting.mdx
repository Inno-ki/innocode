...
Da biste otklonili probleme s InnoCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\innocode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `innocode --log-level DEBUG`.

---

## Pohrana

InnoCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/innocode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\innocode`
...
InnoCode Desktop pokreće lokalni InnoCode server (`innocode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `InnoCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/innocode/innocode.jsonc` (ili `~/.config/innocode/innocode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/innocode/innocode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\innocode\innocode.jsonc`
...
InnoCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/innocode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\innocode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga InnoCode mogao ponovo izgraditi.

1. Potpuno zatvorite InnoCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/innocode`
- **Linux**: obrišite `~/.cache/innocode` (ili pokrenite `rm -rf ~/.cache/innocode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\innocode`

3. Ponovo pokrenite InnoCode Desktop.

---

### Rješavanje problema sa vezom na serveru

InnoCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `innocode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `INNOCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `INNOCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, InnoCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa InnoCode karakteristikama.

---

### Obavještenja se ne prikazuju

InnoCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za InnoCode u postavkama vašeg OS-a, i
...
1. Zatvorite InnoCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka InnoCode Desktop aplikacije):

- `innocode.settings.dat` (URL zadanog servera za desktop)
- `innocode.global.dat` i `innocode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `innocode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\innocode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. InnoCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\innocode`

2. Ponovo pokrenite InnoCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti InnoCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
InnoCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
