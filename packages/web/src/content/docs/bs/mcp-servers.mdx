...
Možete dodati vanjske alate u InnoCode koristeći _Model Context Protocol_, ili MCP. InnoCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [InnoCode Config](https://innocode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/innocode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
InnoCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, InnoCode će:
...
Ako server zahtijeva autentifikaciju, InnoCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `innocode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, InnoCode sigurno čuva tokene u `~/.local/share/innocode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u InnoCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz InnoCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete InnoCode sa vašim Sentry nalogom.
