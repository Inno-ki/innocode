...
U InnoCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, InnoCode koristi nasu `innocode` temu.
...
InnoCode dolazi sa vise ugradenih tema.
...
- Zele da InnoCode odgovara izgledu njihovog terminala
...
InnoCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/innocode/themes/*.json` ili `$XDG_CONFIG_HOME/innocode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.innocode/themes/*.json`
4. **Trenutni radni direktorij** - `./.innocode/themes/*.json`
