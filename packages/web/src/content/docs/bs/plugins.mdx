---
title: Dodaci
description: Napišite vlastite dodatke za proširenje InnoCode.
---

Dodaci vam omogućavaju da proširite InnoCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja InnoCode.
...
- `.innocode/plugins/` - Dodaci na nivou projekta
- `~/.config/innocode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/innocode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/innocode/innocode.json`)
2. Konfiguracija projekta (`innocode.json`)
3. Globalni direktorij dodataka (`~/.config/innocode/plugins/`)
4. Direktorij dodataka projekta (`.innocode/plugins/`)
...
InnoCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: InnoCode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje InnoCode.
...
:::note
Ako koristite desktop aplikaciju InnoCode, ona može automatski slati sistemske obavijesti kada je odgovor spreman ili kada dođe do greške u sesiji.
:::
...
Spriječite innocode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u innocode:
...
Pomoćnik `tool` kreira prilagođeni alat koji innocode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za innocode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://innocode.io/docs/sdk) za detalje.
