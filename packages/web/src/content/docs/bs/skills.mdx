...
Agent skills omogucavaju InnoCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
InnoCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.innocode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/innocode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, InnoCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.innocode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/innocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.innocode/skills/git-release/SKILL.md` ovako:
...
InnoCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `innocode.json`:
...
**Za ugradene agente** (u `innocode.json`):
