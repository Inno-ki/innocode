...
Postoje dvije vrste agenata u InnoCode: primarni agenti i podagenti.
...
InnoCode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
InnoCode dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

InnoCode dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `innocode.json`:
...
- Globalno: `~/.config/innocode/agents/`
- Po projektu: `.innocode/agents/`
...
Ako temperatura nije navedena, InnoCode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu InnoCode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj InnoCode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [InnoGPT](/docs/zen), koristili biste `innocode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `innocode models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/innocode).
:::
