...
ove ili konfigurirajte svoje putem innocode config.
...
innocode dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.innocode/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `innocode.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/innocode/modes/`
- Projekat: `.innocode/modes/`
...
Ako temperatura nije navedena, innocode koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.innocode/modes/` za specifične načine rada ili `~/.config/innocode/modes/` za globalne načine:
