---
title: Server
description: Komunicirajte s innocode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `innocode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji innocode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `innocode.local` |
...
Postavite `INNOCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `innocode`, ili postavite `INNOCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `innocode serve` i za `innocode web`.
...
Kada pokrenete `innocode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite innocode server za programsku interakciju sa innocode-om.
:::

Ova arhitektura omogucava innocode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `innocode serve` da startate standalone server. Ako je innocode TUI vec pokrenut, `innocode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste InnoCode [IDE](/docs/ide) pluginovi.
...
innocode server izlaže sljedece API-je.
