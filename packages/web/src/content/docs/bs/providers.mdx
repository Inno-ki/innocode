---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u InnoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

InnoCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj InnoCode konfiguraciji.
...
u `~/.local/share/innocode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem InnoCode
config.
...
InnoGPT je lista modela koje je obezbedio InnoCode tim koji su bili
testirano i potvrđeno da dobro radi sa InnoCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa InnoGPT.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite innocode i idite na [app.innogpt.de](https://app.innogpt.de).
...
Radi kao i svaki drugi provajder u InnoCode i potpuno je opcionalan za korištenje.
...
Da biste koristili Amazon Bedrock s InnoCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete innocode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `innocode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u InnoCode.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi innocode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju innocode konfiguraciju.
...
da pohrani token u innocode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
InnoCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate InnoCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`innocode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [innocode-gitlab-auth](https://www.npmjs.com/package/@gitlab/innocode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `innocode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s innocode:
...
Za korištenje Google Vertex AI s InnoCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće innocode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem innocodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [innocode-helicone-session](https://github.com/H2Shami/innocode-helicone-session) da automatski prijavite svaki InnoCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki InnoCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati innocode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati innocode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati innocode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za InnoCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/innocode) za detalje.
:::
...
Da biste koristili Ollama Cloud s InnoCode:
...
3. Kopirajte API ključ za korištenje u InnoCode.
...
6. **Važno**: Prije upotrebe modela oblaka u InnoCode, morate lokalno povući informacije o modelu:
...
InnoGPT je lista testiranih i verifikovanih modela koju je obezbedio InnoCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>InnoGPT</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **InnoGPT**.

```txt
   /connect
```

3. Unesite svoj InnoCode API ključ.
...
Također možete dodati dodatne modele putem vaše innocode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše innocode konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa InnoCodeom:
...
Također možete prilagoditi modele kroz svoju innocode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s innocode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `innocode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju InnoCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `innocode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite InnoCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj innocode konfiguraciji.
