...
The config file has a schema that's defined in [**`innocode.ai/config.json`**](https://innocode.ai/config.json).
...
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `innocode.local`. Useful for running multiple instances on the same network.
