...
Para depurar problemas con InnoCode, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\innocode\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `innocode --log-level DEBUG`.

---

## Almacenamiento

innocode almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/innocode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\innocode`
...
InnoCode Desktop ejecuta un servidor InnoCode local (el sidecar `innocode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `InnoCode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/innocode/innocode.jsonc` (o `~/.config/innocode/innocode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/innocode/innocode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\innocode\innocode.jsonc`
...
InnoCode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/innocode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\innocode\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que InnoCode pueda reconstruirlo.

1. Salga de InnoCode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/innocode`
- **Linux**: eliminar `~/.cache/innocode` (o ejecutar `rm -rf ~/.cache/innocode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\innocode`

3. Reinicie el escritorio InnoCode.

---

### Solucionar problemas de conexión del servidor

InnoCode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `innocode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `INNOCODE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `INNOCODE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio InnoCode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de InnoCode.

---

### Notificaciones que no se muestran

InnoCode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para InnoCode en la configuración de su sistema operativo, y
...
1. Salga del escritorio InnoCode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio InnoCode):

- `innocode.settings.dat` (URL del servidor predeterminado de escritorio)
- `innocode.global.dat` y `innocode.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con InnoCode:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/innocode/issues**](https://github.com/anomalyco/innocode/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**innocode.ai/discord**](https://innocode.ai/discord)
...
3. Asegúrese de tener la última versión con `innocode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `innocode models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\innocode`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. innocode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\innocode`

2. Reinicie innocode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a innocode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
innocode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
