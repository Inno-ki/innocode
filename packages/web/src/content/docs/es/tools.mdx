...
Las herramientas permiten que LLM realice acciones en su código base. InnoCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en InnoCode.
...
:::note
Esta herramienta solo está disponible cuando `INNOCODE_EXPERIMENTAL_LSP_TOOL=true` (o `INNOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor InnoCode o cuando la variable de entorno `INNOCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar InnoCode:

```bash
INNOCODE_ENABLE_EXA=1 innocode
```
