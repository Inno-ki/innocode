---
title: Servidor
description: Interactuar con el servidor innocode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `innocode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente innocode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `innocode.local` |
...
Configure `INNOCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `innocode`, o configure `INNOCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `innocode serve` como a `innocode web`.
...
Cuando ejecuta `innocode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor innocode para interactuar con innocode mediante programación.
:::

Esta arquitectura permite que innocode admita múltiples clientes y le permite interactuar con innocode mediante programación.

Puede ejecutar `innocode serve` para iniciar un servidor independiente. Si tienes el
innocode TUI ejecutándose, `innocode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos InnoCode [IDE](/docs/ide).
...
El servidor innocode expone las siguientes API.
