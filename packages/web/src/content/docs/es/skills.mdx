...
Las habilidades del agente permiten a InnoCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
InnoCode busca estas ubicaciones:

- Configuración del proyecto: `.innocode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/innocode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, InnoCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.innocode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/innocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.innocode/skills/git-release/SKILL.md` así:
...
InnoCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `innocode.json`:
...
**Para agentes integrados** (en `innocode.json`):
