...
Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad innocode sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `innocode`).
...
Esto inicia un servidor HTTP y abre un navegador web para acceder a InnoCode a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `innocode`).
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `innocode`) |
