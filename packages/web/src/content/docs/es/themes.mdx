...
Con InnoCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, InnoCode usa nuestro propio tema `innocode`.
...
InnoCode viene con varios temas integrados.
...
- Quiere que InnoCode coincida con la apariencia de su terminal
...
InnoCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/innocode/themes/*.json` o `$XDG_CONFIG_HOME/innocode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.innocode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.innocode/themes/*.json`
