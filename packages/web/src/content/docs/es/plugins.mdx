---
title: Complementos
description: Escriba sus propios complementos para extender InnoCode.
---

Los complementos le permiten extender InnoCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de InnoCode.
...
- `.innocode/plugins/` - Complementos a nivel de proyecto
- `~/.config/innocode/plugins/` - Complementos globales
...
```json title="innocode.json"
{
  ...
  "plugin": ["innocode-helicone-session", "innocode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/innocode/node_modules/`.
...
1. Configuración global (`~/.config/innocode/innocode.json`)
2. Configuración del proyecto (`innocode.json`)
3. Directorio global de complementos (`~/.config/innocode/plugins/`)
4. Directorio de complementos del proyecto (`.innocode/plugins/`)
...
```json title=".innocode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
InnoCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".innocode/plugins/my-plugin.ts"
...
```js title=".innocode/plugins/example.js"
...
- `client`: Un cliente SDK innocode para interactuar con la IA.
...
import type { Plugin } from "@innocode-ai/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar innocode.
...
```js title=".innocode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "innocode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio InnoCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que innocode lea archivos `.env`:

```javascript title=".innocode/plugins/env-protection.js"
...
```javascript title=".innocode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a innocode:

```ts title=".innocode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@innocode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que innocode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para innocode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://innocode.ai/docs/sdk) para obtener más detalles.
...
```ts title=".innocode/plugins/compaction.ts"
import type { Plugin } from "@innocode-ai/plugin"
...
```ts title=".innocode/plugins/custom-compaction.ts"
import type { Plugin } from "@innocode-ai/plugin"
