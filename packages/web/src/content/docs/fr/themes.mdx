...
Avec InnoCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, InnoCode utilise notre propre thème `innocode`.
...
InnoCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que InnoCode corresponde à l'apparence de leur terminal
...
InnoCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/innocode/themes/*.json` ou `$XDG_CONFIG_HOME/innocode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.innocode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.innocode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/innocode/themes
vim ~/.config/innocode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .innocode/themes
vim .innocode/themes/my-theme.json
```
...
{
  "$schema": "https://innocode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
