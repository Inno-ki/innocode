...
Il existe deux types d'agents dans InnoCode ; agents primaires et sous-agents.
...
InnoCode est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
InnoCode est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

InnoCode est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `innocode.json` :
...
- Global : `~/.config/innocode/agents/`
- Par projet : `.innocode/agents/`

```markdown title="~/.config/innocode/agents/review.md"
---
...
Si aucune température n'est spécifiée, InnoCode utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale InnoCode et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration InnoCode utilise le format `provider/model-id`. Par exemple, si vous utilisez [InnoGPT](/docs/zen), vous utiliserez `innocode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="innocode.json" {3-6,9-12}
{
  "$schema": "https://innocode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="innocode.json" {3-5,8-10}
{
  "$schema": "https://innocode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `innocode models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/innocode).
:::
...
```markdown title="~/.config/innocode/agents/docs-writer.md"
---
...
```markdown title="~/.config/innocode/agents/security-auditor.md"
---
