---
title: Plugins
description: Écrivez vos propres plugins pour étendre InnoCode.
---

Les plugins vous permettent d'étendre InnoCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de InnoCode.
...
- `.innocode/plugins/` - Plugins au niveau du projet
- `~/.config/innocode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/innocode/node_modules/`.
...
1. Configuration globale (`~/.config/innocode/innocode.json`)
2. Configuration du projet (`innocode.json`)
3. Répertoire global des plugins (`~/.config/innocode/plugins/`)
4. Répertoire des plugins du projet (`.innocode/plugins/`)
...
InnoCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK innocode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre innocode.
...
:::note
Si vous utilisez l'application de bureau InnoCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez innocode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à innocode :
...
L'assistant `tool` crée un outil personnalisé que innocode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour innocode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://innocode.ai/docs/sdk) pour plus de détails.
