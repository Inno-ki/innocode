---
title: SDK
description: Client JS de type sécurisé pour le serveur innocode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK innocode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler innocode par programme.
...
Créez une instance de innocode :

```javascript
import { createOpencode } from "@innocode-ai/sdk"
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `innocode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
Si vous disposez déjà d'une instance en cours d'exécution de innocode, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createOpencodeClient } from "@innocode-ai/sdk"
...
```typescript
import type { Session, Message, Part } from "@innocode-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*innocode" },
})
