...
description: Créez des outils que le LLM peut appeler dans innocode.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de innocode comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.innocode/tools/` de votre projet.
- Ou globalement, en les plaçant dans `~/.config/innocode/tools/`.
...
```ts title=".innocode/tools/database.ts" {1}
import { tool } from "@innocode-ai/plugin"
...
```ts title=".innocode/tools/math.ts"
import { tool } from "@innocode-ai/plugin"
...
```ts title=".innocode/tools/project.ts" {8}
import { tool } from "@innocode-ai/plugin"
...
```python title=".innocode/tools/add.py"
import sys
...
```ts title=".innocode/tools/python-add.ts" {10}
import { tool } from "@innocode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".innocode/tools/add.py")
