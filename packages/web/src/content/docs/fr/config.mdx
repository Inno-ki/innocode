...
Spécifiez un répertoire de configuration personnalisé à l'aide de `OPENCODE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.innocode`, et devrait suivre la même structure.
...
Le fichier de configuration a un schéma défini dans [**`innocode.ai/config.json`**](https://innocode.ai/config.json).
