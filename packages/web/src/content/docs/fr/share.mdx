---
title: Partager
description: Partagez vos conversations InnoCode.
---

La fonctionnalité de partage de InnoCode vous permet de créer des liens publics vers vos conversations InnoCode, afin que vous puissiez collaborer avec vos coéquipiers ou obtenir de l'aide d'autres personnes.
...
Lorsque vous partagez une conversation, InnoCode :

1. Crée un URL public unique pour votre session
2. Synchronise l'historique de vos conversations avec nos serveurs
3. Rend la conversation accessible via le lien partageable — `innocode.io/s/<share-id>`

---

## Partage

InnoCode prend en charge trois modes de partage qui contrôlent la manière dont les conversations sont partagées :

---

### Manuel (par défaut)

Par défaut, InnoCode utilise le mode de partage manuel. Les sessions ne sont pas partagées automatiquement, mais vous pouvez les partager manuellement à l'aide de la commande `/share` :
...
Pour appliquer cela au sein de votre équipe pour un projet donné, ajoutez-le au `innocode.json` dans votre projet et enregistrez-vous dans Git.
...
[En savoir plus](/docs/enterprise) sur l'utilisation de innocode dans votre organisation.
