---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um InnoCode zu erweitern.
---

Mit Plugins können Sie InnoCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von InnoCode zu ändern.
...
- `.innocode/plugins/` – Plugins auf Projektebene
- `~/.config/innocode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/innocode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/innocode/innocode.json`)
2. Projektkonfiguration (`innocode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/innocode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.innocode/plugins/`)
...
InnoCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein InnoCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie InnoCode erweitern können.
...
:::note
Wenn Sie die InnoCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass InnoCode `.env`-Dateien liest:
...
Plugins können InnoCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das InnoCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für InnoCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://innocode.ai/docs/sdk).
