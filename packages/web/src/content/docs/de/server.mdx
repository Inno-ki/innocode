---
title: Server
description: Nutze den innocode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `innocode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein innocode-Client nutzen kann.
...
Setze `INNOCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `innocode`, kann aber mit `INNOCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `innocode serve` und `innocode web`.
...
Wenn du `innocode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den innocode-Server, um innocode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `innocode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `innocode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die InnoCode-[IDE](/docs/ide)-Plugins.
...
Der innocode-Server stellt folgende APIs bereit.
