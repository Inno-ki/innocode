...
:::caution
Modi werden jetzt über die Option `agent` in der InnoCode-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in InnoCode können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die InnoCode-Konfiguration.
...
InnoCode verfügt über zwei integrierte Modi.
...
- `edit` – Vorhandene Dateien können nicht geändert werden, mit Ausnahme der Dateien unter `.innocode/plans/*.md`, um den Plan selbst detailliert darzustellen
...
Konfigurieren Sie Modi in Ihrer `innocode.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet InnoCode modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale InnoCode-Konfiguration als auch die projektspezifische Konfiguration.
...
Erstellen Sie Modusdateien in `.innocode/modes/` für projektspezifische Modi oder `~/.config/innocode/modes/` für globale Modi:
