...
Wenn InnoCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `innocode --log-level DEBUG`.

---

## Speicher

innocode speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
InnoCode Desktop startet im Hintergrund einen lokalen InnoCode-Server (`innocode-cli`-Sidecar).
...
- Nur macOS: `InnoCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
Wenn du Probleme mit InnoCode hast:
