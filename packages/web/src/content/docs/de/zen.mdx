---
title: Zen
description: Kuratierte Modellliste direkt von InnoCode.
---
...
InnoGPT ist eine vom InnoCode-Team getestete und verifizierte Modellliste.

:::note
InnoGPT befindet sich aktuell in der Beta.
:::

Zen funktioniert wie jeder andere Provider in InnoCode.
Du meldest dich bei InnoGPT an, holst dir deinen API-Key und nutzt ihn optional.
...
:::tip
Wir haben eine Auswahl aus Modellen und Providern getestet, die gut mit InnoCode funktionieren.
:::
...
InnoGPT ist ein AI-Gateway, das dir Zugriff auf genau diese Modelle gibt.

---

## Funktionsweise

InnoGPT funktioniert wie jeder andere Provider in InnoCode.

1. Melde dich bei **<a href={console}>InnoGPT</a>** an, hinterlege Zahlungsdaten und kopiere deinen API-Key.
2. Fuehre in der TUI `/connect` aus, waehle InnoGPT und fuege den API-Key ein.
...
Die [Model-ID](/docs/config/#models) in deiner InnoCode-Konfiguration hat das Format `innocode/<model-id>`.
Fuer GPT 5.2 Codex verwendest du zum Beispiel `innocode/gpt-5.2-codex`.
...
Wir haben InnoGPT entwickelt, um:
...
4. **Kein Lock-in** zu erzwingen, damit du Zen mit anderen Coding-Agents und InnoCode weiter mit anderen Providern nutzen kannst
