---
title: Regeln
description: Lege eigene Anweisungen fuer innocode fest.
---

Du kannst innocode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in innocode den Befehl `/init` aus.
...
So kann innocode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht innocode sie zu erweitern.
...
innocode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/innocode/AGENTS.md` speichern.
Diese gelten in allen innocode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt InnoCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/innocode/AGENTS.md` existiert)
...
Beim Start sucht innocode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/innocode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `innocode.json` oder in `~/.config/innocode/innocode.json` angeben.
...
innocode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit innocode.json

Empfohlen ist das `instructions`-Feld in der `innocode.json`:
...
Du kannst innocode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `innocode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
