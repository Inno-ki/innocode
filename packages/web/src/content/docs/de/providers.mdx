---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in InnoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

InnoCode verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer InnoCode-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem InnoCode anpassen
config.
...
InnoGPT ist eine Liste von Modellen, die vom InnoCode-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit InnoCode funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit InnoGPT zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie InnoCode aus und gehen Sie zu [innocode.ai/auth](https://innocode.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in InnoCode und ist völlig optional.
...
So verwenden Sie Amazon Bedrock mit InnoCode:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie InnoCode ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `innocode.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in InnoCode wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit InnoCode ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre InnoCode-Konfiguration hinzufügen.
...
um das Token im InnoCode-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
InnoCode verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von InnoCode
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`innocode.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [innocode-gitlab-auth](https://www.npmjs.com/package/@gitlab/innocode-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `innocode.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit InnoCode:
...
So verwenden Sie Google Vertex AI mit InnoCode:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie InnoCode ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über InnoCode konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [innocode-helicone-session](https://github.com/H2Shami/innocode-helicone-session)-Plugin, um jede InnoCode-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede InnoCode-Konversation als separate Sitzung aufgeführt.
...
Sie können InnoCode für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können InnoCode über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können InnoCode über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für InnoCode konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/innocode).
:::
...
So verwenden Sie Ollama Cloud mit InnoCode:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in InnoCode.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in InnoCode verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
InnoGPT ist eine Liste getesteter und verifizierter Modelle, die vom InnoCode-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>InnoGPT</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **InnoGPT**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren InnoCode API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre InnoCode-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre InnoCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit InnoCode:
...
Sie können Modelle auch über Ihre InnoCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit InnoCode verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `innocode.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann InnoCode erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `innocode auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die InnoCode-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer InnoCode-Konfiguration übereinstimmt.
