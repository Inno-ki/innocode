...
Agent Skills erlauben InnoCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
InnoCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft InnoCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.innocode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/innocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.innocode/skills/git-release/SKILL.md` zum Beispiel so:
...
InnoCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `innocode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `innocode.json`):
